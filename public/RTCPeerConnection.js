...
var endPoint = 'ws://localhost:8000/ws/chat/lobby/'
...
function createOfferer(peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name) {
    ...
    if (!localScreenSharing && !remoteScreenSharing) {
        ...
        //createScreen
        remoteVideo = createScreen(peerUsername);
        setOnTrack(peer, remoteVideo);
        console.log('Remote video source: ', remoteVideo.srcObject);
...
function createAnswerer(offer, peerUsername, localScreenSharing, remoteScreenSharing, receiver_channel_name) {
    ...
    if (!localScreenSharing && !remoteScreenSharing) {
        ...
        // set remote video, createScreen
        var remoteVideo = createScreen(peerUsername);
        // and add tracks to remote video
        setOnTrack(peer, remoteVideo);
...
function createVideo(peerUsername) {
    ...
    remoteVideo.className = "client";
    remoteVideo.width = 200;
    remoteVideo.height = 200;
    remoteVideo.onclick = () => {
        if (userScreenShared === false) {
            screen = document.getElementById(`${peerUsername}-screen-video`);
            screen.width = 300;
            screen.height = 200;
            screen.play();
            userScreenShared = true;
        } else {
            screen = document.getElementById(`${peerUsername}-screen-video`);
            screen.width = 1;
            screen.height = 1;
            screen.pause();
            userScreenShared = false;
        }
    }
...
function createScreen(peerUsername) {
    var videoContainer = document.getElementById(`${peerUsername}`);
    ...
    remoteVideo.id = peerUsername + '-screen-video';
    remoteVideo.autoplay = true;
    remoteVideo.playsinline = true;
    remoteVideo.className = "client-screen";
    remoteVideo.width = 1;
    remoteVideo.height = 1;
    remoteVideo.pause();
